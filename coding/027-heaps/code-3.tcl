...
array set previousHeap [array get heap]

if {$random > 0} {
    for {set i 1} {$i <= $random} {incr i} {
        permuteHeapMain heap [expr {int([ns_rand] * $random * 20)}]]
    }
} elseif {[llength $y] > 0} {
    foreach element $y {
        permuteHeapMain heap $element false
    }
}
...
<pre>
previousHeap = '[array get previousHeap]'
h = '$h'
y = '$y'
verified = '$verified'
