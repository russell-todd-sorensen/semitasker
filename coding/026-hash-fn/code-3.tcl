...
proc tclHash {str size} {
    
    # in tclHash.c as 0x811c9dc5 = 1000 0001 0001 1100 1001 1101 1100 0101
    set hash 2166136261
    foreach char [split $str ""] {
        set hash [expr { (($hash<<3) + [scan $char %c])}]
        #ns_log Notice "hash for $str now $hash"
    }
    return [expr {$hash % $size}] 
}

proc tclHashDJB2 {str size} {

    set hash 5381
    foreach char [split $str ""] {
        set hash [expr { ((($hash<<5) + $hash ) + [scan $char %c])}]
        #ns_log Notice "hash for $str now $hash"
    }
    return [expr {$hash % $size}]
}
...
proc tclHashs {keys {fid 0} {size 100}} {
    global hashFn
    foreach key $keys {
        set hash [$hashFn($fid) $key $size]
        lappend hashs $hash
    }
    return $hashs
}
...
set buckets [tclHashs $keys $fid $size]
...
<ul>
 ...
 <li>
  <label for='s'>Hash Table Size </label>
