...
set statsInterpName stats 
set raceInterpName  race 
...
deleteInterp $raceInterpName

set raceInterp [interp create $raceInterpName]
...
interp eval $raceInterp {
    ...
    global log statsInterpName statsInterp
    set log [list]
    set statsInterpName stats
    set statsInterp [interp create $statsInterpName]
    ...
    proc init {} {
        global statsInterp
        initLog
        interp eval $statsInterp racingTimesMinMax 1 100
        interp eval $statsInterp initHorses
        interp eval $statsInterp getHorses
    ...
    proc initRaceInterp {statsInterpName} {
        global statInterp 
        set statsInterp $statsInterpName
    }
    ...
    global raceHeatLetters heatIndex raceHeats
    set heatIndex 0
    set raceHeatLetters "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    set raceHeats [list]

    # Note: min heat size is one more than number of top horses to find
    proc createRandomRaceHeats {{arrayName horse} {heatSizeMax 5} {minHeatSize 4}} {
        global raceHeatLetters heatIndex statsInterp raceHeats
        upvar $arrayName horse 
        set horseNameList [interp eval $statsInterp getHorses]
        while {[llength $horseNameList] > $minHeatSize} {
            ...
            for {set i 0} {$i < $heatSizeMax && [llength $horseNameList]>0} {incr i} {
                set index [expr {floor([ns_rand]*[llength $horseNameList])}]
...
interp eval $raceInterp { interp eval $statsInterp {
...
set result [interp eval $raceInterp init]
