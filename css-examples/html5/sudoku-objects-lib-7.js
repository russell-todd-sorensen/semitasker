...
function getMask() {
	return (this.row & this.col & this.box);
}

function removeFromMasks() {
	//remove value from masks
	this.row &= ~(1<<this.value);
	this.col &= ~(1<<this.value);
	this.box &= ~(1<<this.value);
}
...
function Cell(id,type,row,col,box) {
	this.id = id;
	this.type = type;
	this.row = row;
	this.col = col;
	...
	this.value = 0;
	this.mask = getMask;
	this.removeFromMasks = removeFromMasks;
	this.addToMasks = addToMasks;
}

// Puzzle will maintain puzzle data
// with three args, boxCols and boxRows is something besides guessed value
// with four args, boxCols, boxRows, plus cell box mapping
function Puzzle(puzzleData) {
	var args = arguments;
	this.boxCols = 3;
	this.boxRows = 3;
	this.boxMapString = "";
	...
	if (args.length > 1) {
		...
		this.boxRows = args[2];
		if (args.length == 4) {
			this.boxMapString = args[3];
			doCellBox = false;
		}
	}
	
	this.puzzleDimension = boxCols * boxRows;
	this.activeCells = this.puzzleDimension * this.puzzleDimension;
	// puzzle will be an array of cell objects.
	this.puzzle = new Array(this.activeCells+3);

	this.boxMap = new Array(0);
}
...
var formatProfiles = new Array(8);
