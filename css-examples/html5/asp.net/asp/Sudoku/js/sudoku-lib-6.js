...
function setupSudoku2() {
    var args = setupSudoku2.arguments;

    if (args.length > 1) {
        boxCols = args[0];
        boxRows = args[1];
        puzzleString = "E" + args[2] + "P";
    } else {
        puzzleString = "E" + args[0] + "P";
    }
    if (boxCols * boxCols * boxRows * boxRows == puzzleString.length-2) {
        return setupSudoku(boxCols, boxRows, boxMapString);
    }
    //var length = puzzleString.length -2;
    //if (length == 36) {
     //   boxCols = 2;
     //   boxRows = 3;
      //  return setupSudoku(boxCols, boxRows, boxMapString);
    //}
    alert("Puzzle has bad length " + length);
    return 0;
}
