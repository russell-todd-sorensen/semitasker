...
var randomizeTime;
var moveIndex;
var moveTileInterval;

function processForm (form) {
  
  puzzleImage = new Image();
  Log.Notice("Processing form...");
  puzzleImage.src = $('#image').val();
  Log.Notice("Image=" + puzzleImage.src);
  var tileDim = $('t#tile-size').val();
  var puzzleRowsNCols = $('#rows-n-cols').val();
  var border = $('#border').val();
  randomizeTime = $('#rand-time').val();
  //image, tileMin, rowsTry, colsTry, border
  setTimeout("generatePuzzle(puzzleImage," 
    + tileDim 
    + "," 
    + puzzleRowsNCols 
    + "," 
...
function generatePuzzle (image, tileMin, rowsTry, colsTry, border) {
  ...
  function randomizePuzzle() {
    ...
    var maxDim = (rows<cols?cols:rows);
    maxDim = (maxDim<4?4:maxDim);
    var origSteps = maxDim*rows*cols;
    var steps = origSteps;
    var rand;
    var dropzone;
    var exclude = [];
    ...
    while (steps > 0) {
      ...
      case 1:  // piece goes w
          if (emptyX - 1 >= 0) {
            ...
            randomMoves[randomMoves.length] = [emptyY, emptyX];
          }
          break;
      case 2: // piece goes e
          if (emptyX+1 < cols) {
    ...
    moveIndex = 0;
    moveTileInterval = setInterval("moveTiles()", Math.ceil(1000*randomizeTime/(origSteps)));
  }
...
function moveTiles() {
  ...
  if (moveIndex >= randomMoves.length) {
    clearInterval(moveTileInterval);
    return;
  }

  tile = randomMoves[moveIndex];
  tileRow = tile[0];
  tileCol = tile[1];
  dropzone = puzzleDropzone[tileRow][tileCol];
  tileId = dropzone.tileId;  
  Log.Notice("In moveTiles() moveIndex=" + moveIndex + " tileId=" + tileId);
  //d3.select('#' + tileId)
    //.on("each", moveTile);
  $('#' + tileId).click();
