...
function hsbRegisterSliderMove (evt) {
  if (evt.data.orientation == 'horizontal') {
    // Log.Notice('hsbRegisterSliderMove new X value=' + parseFloat(evt.data.currentX+evt.data.skewX));
  } else if (evt.data.orientation == 'vertical') {
    // Log.Notice('hsbRegisterSliderMove new Y value=' + parseFloat(evt.data.currentY+evt.data.skewY));
  }
}
...
function hsbMainSwatchUpdate(evt) {
  ...
  case 'hue':
	...
	d3.select('#hsb-hue').attr('fill',swatch.hsbHue());
	d3.select('#hsb-sat').attr('fill',swatch.hsbSat());
	d3.select('#hsb-brt').attr('fill',swatch.hsbBrt());
...
var Swatch = function (name, swatchIndex) {
  ...
  this.hsbSat = function () {
		var satInverse = 10*(100 - this.sat)/1000;
		return "rgba(255,255,255," + satInverse + ")";
	};
  this.hsbBrt = function () {
		var brtInverse = 10*(100 - this.brt)/1000;
    ...
    for (var i = 0; i < this.hsbSliders.length; i++) {
      currentSlider = sliders[this.hsbSliders[i]];
			...
			for (var k = 0; k< currentSlider.gradientStack.length;k++) {
				...
				case 'filled-rect':
				  $('#' + gradStack.id).attr('fill', this[gradStack.colorFn]() );
				  break;
				case 'fixed-gradient':
				  break;
				default:
...
var hsbSlider = function (sliderIndex, swatch, component) {
  ...
  this.call = [hsbRegisterSliderMove,hsbColorChange];
...
function writeSvg(appSelector) {
  ...
  for (var i = 0; i<swatches.length; i++) {
    ...
    var swatchGroup = svg
        ...
        .attr({
            ...
            + parseFloat(sliderInfo.swatchLeftSpacing 
						+ ( sliderInfo.displayWidth + sliderInfo.swatchSpacing) * swatchIndex) 
            + "," 
    ...
    for (var component in swatch.hsbComponents) {
      ...
      case 'sat':
        slider.stops = [
          {id: sliderIndex, offset: 0, "stop-color": "white", "stop-opacity": "100%"},
          {id: sliderIndex, offset: 100, "stop-color": "white", "stop-opacity": "0%"}
        ];
        slider.fullScaleValue = 100;
        slider.minX = 0;
        slider.maxX = 100;
      ...
      case 'brt':
        slider.stops = [
          {id: sliderIndex, offset: 0, "stop-color": "black", "stop-opacity": "100%"},
          {id: sliderIndex, offset: 100, "stop-color": "black", "stop-opacity": "0%"}
        ];
        slider.fullScaleValue = 100;
        slider.minX = 0;
        slider.maxX = 100;
...
function generateTC( ) {
	...
	var theory = $('#theory option::selected').val();
	var angle = $('#angle').val();
	//$('#hsl-swatch-current').attr('fill',baseColor);
	//$('#angle-display').html(angle);
  swatches[5].newColor(baseColor);
