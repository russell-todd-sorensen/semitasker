...
source [file join $absolutePath load-only-customers.tcl]
...
set form "<form id='form1' name='form1' action='select-customer-2.tcl'>
...
foreach field $customerFields {
  ...
  [join $customerOptionList \n]
  		...
  		|| [lsearch -exact {CTYPE JOBTYPE TERMS PRICELEVEL REFNUM TIMESTAMP PAYMETH TAXITEM SALESTAXCODE REP JOBSTATUS TAXABLE LIMIT} $field] > -1 } {
		#lappend formFields "<li><label for='field_$field'>$fieldLabel</label>\n\[createSelect $field\]</li>"
		lappend formFields "<li><label for='field_$field'>$fieldLabel</label>
		    <input id='field_$field' name='field_$field' type='hidden'>
		    <span id='span_field_$field'></span>\n</li>"
