...
foreach participant [lsort [array names CUST]] {
	...
	set fInvNumber "$month-[format %0.4d $invoiceNumber]"
        lappend invoiceLines [list TRNS "" INVOICE $invoiceDate "Accounts Receivable" [set $nameField] "" $fees $fInvNumber $memo N Y N "$BADDR2" "$BADDR3" "$terms" "" $invoiceDate $invoiceDate]
	incr invoiceNumber
	#lappend invoiceLines [list TRNS "" INVOICE $invoiceDate "Accounts Receivable" [set $nameField] "" $fees $memo N Y N "" "" "$terms" "" $invoiceDate]
