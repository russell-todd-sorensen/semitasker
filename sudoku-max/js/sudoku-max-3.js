...
var defaultSymbolsProto = "123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcd";
var defaultSymbols = "" + defaultSymbolsProto;
//var symbolIndexValues = "123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcd";
...
var resetMaskValues = function (cellIndex) {
	//cellValue = symbolIndex[puzzle[cellIndex]];
...
var toggleExtraInfo = function(id,ref) {
    ...
    if (extraInfo) {
        $('#' + id).hide();
        $('#' + buttonId).html("Show More");
        extraInfo = false;
    } else {
        $('#' + id).show();
        $('#' + buttonId).html("Show Less");
        extraInfo = true;
    }
    return false;
}
