...
ad_proc -public qw_new { 
    ...
    ns_log Debug "qw_new: object '$OBJECT' array '$ARRAY' Files: '$FILES'"
    upvar $ARRAY ARR
...
ad_proc -private qw_new_pseudo_postgresql { } {
	...
	if {[lsearch $ACS_OBJECT_ATTRS $REAL_ATTR] > -1} {
      ...
      if {[nsv_exists qw_to_eval $OBJECT]} {
        ns_log Debug "FOUND EVAL for '$OBJECT'"
	  foreach {VAR EVAL_STMT} [split [nsv_get qw_to_eval $OBJECT] ";" ] {
	      ns_log Debug "EVAL: '$EVAL_STMT' VAR: '$VAR'"
	      eval $EVAL_STMT
...
proc ::upload_files {files array} {
	...
	ns_log Notice "Got file for '$attribute' original_file_var: '$original_file_var'"
	...
	set backslash "\\"
	ns_log Notice "backslash: '$backslash'"
	...
	ns_log Notice "tmpfile is '$tmpfile' filesize: $filesize content-type: $content_type
content-disposition: '$content_disposition'
filename: '$filename'"
	ns_log Notice "Key: $object_key value: $Array($object_key)"
...
proc ::qw::filter::combine_date_parts { 
	...
	foreach {PART FMT} [list year %Y month %m day %d hour %H minute %M second %S timezone %Z] {
	    ...
	    ns_log Notice "DATE PART: '$PART' = '[set $PART]'"
	}
...
order by
 ...
 callback_order" {
     ...
     ns_log Notice "qwic: init qw_callbacks_${operation}_${object_id} $callback_point"
...
proc qw::callbacks::run { object_id operation callback_point } {
	...
	foreach callback [nsv_get qw_callbacks_${operation}_${object_id} $callback_point] {
	    ns_log Notice "callback ${object_id}_${operation}_${callback_point}: '$callback'"
	    uplevel $callback
	}
    }
}
