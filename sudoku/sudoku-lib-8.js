...
var boxCols = 1;
var boxRows = 5;
var puzzleString = "E1234500000000000000000453P";
...
// Arrays to hold media playlist and sequence to play them in
var mediaArray = new Array();
var sequenceArray = new Array();
var sequenceId = 0;

function media( filename, type, id ) {
  Log("Notice", "media: making media object filename = '" + filename + "' type = '" + type + "' id = '" + id + "'");
  this.filename = filename;
  this.type = type;
  this.id = id;
}

function addSequence (mediaId, duration) {
  Log("Notice", "addSequence: mediaId: '" + mediaId + "' duration: '" + duration + "'");
  sequenceArray[parseInt(sequenceId)] = [parseInt(mediaId), parseInt(duration)];
  sequenceId++;
}

function setupMedia() {
  mediaArray[0] = new media('my-motion.gif', 'img', 'P1');
  mediaArray[1] = new media('my-cropped.jpeg', 'img', 'P1');
  mediaArray[2] = new media('hand.mp4', 'video', 'P1');
  mediaArray[3] = new media('hand.gif', 'img', 'P1');
  mediaArray[4] = new media('solving-1-dv.gif', 'img', 'P2');
  mediaArray[5] = new media('solving-2-dv.gif', 'img', 'P2');
  mediaArray[6] = new media('solving-3-dv.gif', 'img', 'P2');
  mediaArray[7] = new media('my-cropped.jpeg', 'img', 'P2');
  mediaArray[8] = new media('solving-3.avi', 'video', 'P2');
  mediaArray[9] = new media('solving-2.avi', 'video', 'P2');
  mediaArray[10] = new media('solving-1.avi', 'video', 'P2');
...
function loadMedia ( sequence ) {

  Log("Notice", "loadMedia: running sequence " + sequence );

  //var seq = parseInt(sequenceArray[parseInt(sequence)]);
  var index = parseInt(sequenceArray[sequence][0]);
  var duration = parseInt(sequenceArray[sequence][1]);

  filename = mediaArray[index].filename;
  type = mediaArray[index].type;
  id = mediaArray[index].id;

  if (type == "img") {
    Log("Notice","loadMedia loading image = '" + filename + "'");
    document.getElementById(id).innerHTML = "<img src='" + filename + "' height='250'>";

  } else if (type == "video") {
    Log("Notice","loadMedia loading video = '" + filename + "'");
    var html = "<video src='" + filename + "'";
    html += "\theight='250' preload='auto' autoplay='autoplay'";
    html += "\tloop='loop' muted='muted' >"
    html += "</video>";
    document.getElementById(id).innerHTML = html;
  }

  sequence++;
  Log("Notice", "loadMedia: duration = " + sequenceArray.length +  " sec" );
  if (sequence < sequenceArray.length) {
 
    setTimeout("loadMedia(" + parseInt(sequence)  + ")", duration * 1000);
  } else {
    //loop
    setTimeout("stopMedia()", duration * 1000);
  }

}

function stopMedia() {
  Log("Notice", "stopMedia: Finished with media list");
}

function loadImage2 (id, filename) {
  Log ("Notice", "In loadImage2");
  document.getElementById(id).innerHTML = "<img src='" + filename + "' height='250'>";
  
}

function loadImage ( id, filename, duration ) {
  Log ("Notice", "In loadImage");
  setTimeout("loadImage2('" + id + "', 'my-cropped.jpeg')" , duration * 1000);
  document.getElementById(id).innerHTML = "<img src='" + filename + "' height='250'>";
  //setTimeout("loadImage2( id, 'my-cropped.jpeg')" , duration * 10);
